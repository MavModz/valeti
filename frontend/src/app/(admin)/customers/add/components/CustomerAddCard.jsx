...
const CustomerAddCard = ({ formData, city, country, uploadedImage = null, isLoading = false }) => {
  ...
  const previewData = {
    ...
    viewProperties: formData?.viewProperties || 0,
    ownProperties: formData?.ownProperties || 0,
    investProperty: formData?.investProperty || 0,
  ...
  // Generate full name
  const fullName = `${previewData.firstName} ${previewData.lastName}`.trim() || 'New Customer';
  ...
  const profileImageAlt = uploadedImage?.originalName || 'Customer Avatar';
        ...
        <CardBody>
          ...
          <div className="d-flex align-items-center gap-2 border-bottom pb-3">
            ...
            <div className="d-block">
              <h6 className={`fw-medium fs-16 mb-1 ${fullName !== 'New Customer' ? 'text-dark' : 'text-muted'}`}>
                {fullName}
              </h6>
              ...
              <p className="mb-0 text-primary">
                {previewData.email ? `# ${previewData.firstName || 'Customer'}` : '# New Customer'}
              </p>
            </div>
          </div>
          ...
          <div className="mt-3 d-flex justify-content-between">
            <div className="text-center">
              <p className="fw-medium mb-1">View Properties</p>
              <h6 className="mb-0 fw-semibold text-dark">{previewData.viewProperties}</h6>
            </div>
            <div className="text-center">
              <p className="fw-medium mb-1">Own Properties</p>
              <h6 className="mb-0 fw-semibold text-dark">{previewData.ownProperties}</h6>
            </div>
            <div className="text-center">
              <p className="fw-medium mb-1">Invest Property</p>
              <h6 className="mb-0 fw-semibold text-dark">{currency}{previewData.investProperty.toLocaleString()}</h6>
            </div>
          </div>
